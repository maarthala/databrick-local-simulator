...
- ⚠️ **Phase 3 (Trino→UC) — not reachable on this OSS build.** UC's Iceberg REST endpoint *works* (the earlier 500 was a wrong path — the correct prefix is `/v1/catalogs/<cat>/…`, which lists namespaces fine). The real blocker is **UniForm**: UC only exposes a table to Trino-as-Iceberg if it carries Iceberg metadata, and enabling UniForm through the UC Spark connector generated none (properties not persisted, no `metadata/` dir). Trino's Delta connector also has no UC-metastore option. So: use Trino on the open `iceberg` catalog (ungoverned) for SQL/BI; Spark-through-UC for governed. Fixing = the fragile UniForm↔UC integration.
