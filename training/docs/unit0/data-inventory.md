...
- **`demo-bucket`** — the lake. Key paths:
  ...
  - **`warehouse/`** — the **warehouse** where the catalog's table files physically live (Bronze/
    Silver/Gold Parquet + Iceberg metadata, e.g. `warehouse/gold/daily_sales`). You normally read
